...
from app.forms import ClassForm, RegistrationForm, LoginForm, EditStudentForm, EmptyForm
...
def index():
    ...
    return render_template('index.html', title="Course List", classes = classes, form=form)
...
def display_profile():
    form = EmptyForm()
    return render_template('display_profile.html', title='Display Profile',student=current_user, form = form)
...
def enroll(classid):
    form = EmptyForm()
    if form.validate_on_submit():
        _class = Class.query.filter_by(id=classid).first()
        if _class is None:
            flash('Class with id "{}" not found.'.format(classid))
            return redirect(url_for('index'))
        
        current_user.enroll(_class)
        db.session.commit()
        flash('You are now enrolled in class {} {}!'.format(_class.major,_class.coursenum))
        return redirect(url_for('index'))
    else:
        return redirect(url_for('index'))
...
def unenroll(classid):
    ...
    if form.validate_on_submit():
        ...
        current_user.unenroll(_class)
        db.session.commit()
        flash('You are now unenrolled from class {} {}!'.format(_class.major,_class.coursenum))
        return redirect(url_for('index'))
    else:
        return redirect(url_for('index'))